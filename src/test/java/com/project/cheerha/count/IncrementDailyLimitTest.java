...
public class IncrementDailyLimitTest {
    ...
    void incrementDailyLimit_첫번째요청_카운트_1증가_만료시간설정() {
        ...
        verify(keyValueCommandRepository, times(1)).incrementValue(REDIS_KEY);
        verify(keyValueCommandRepository, times(1)).expireValue(eq(REDIS_KEY), anyLong(), eq(TimeUnit.SECONDS));
    }

    @Test
    void incrementDailyLimit_네번째요청_예외발생() {
        when(keyValueQueryRepository.getValue(REDIS_KEY)).thenReturn("3");
        ...
        verify(keyValueCommandRepository, never()).incrementValue(REDIS_KEY);
    ...
    void incrementDailyLimit_첫요청일때만_expire_설정() {
        ...
        verify(keyValueCommandRepository, times(1)).expireValue(eq(REDIS_KEY), eq(expectedExpiration), eq(TimeUnit.SECONDS));
    }
