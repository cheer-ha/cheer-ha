...
public class MappingService {
    ...
    public void saveMappings(
        ...
        emailToUrl.forEach((email, urlSet) -> {
            ...
            List<UserToJobOpeningMapping> foundUserToJobOpeningMappingList = userToJobOpeningMappingRepository.findAllByEmailAndJobOpeningUrlIn(
                email,
            ...
            // 중복을 확인할 때 사용
            Set<String> existingUrlSet = foundUserToJobOpeningMappingList.stream()
                .map(UserToJobOpeningMapping::getJobOpeningUrl)
                .collect(Collectors.toSet());

            // 중복 확인 후 기존에 없는 Mapping 객체만 생성
            // .filter : 중복 제거
            // .map: Mapping 객체 생성
            List<UserToJobOpeningMapping> userToJobOpeningMappingList = urlSet.stream()
                .filter(jobOpeningUrl ->
                    !existingUrlSet.contains(jobOpeningUrl)
                ).map(jobOpeningUrl ->
                    UserToJobOpeningMapping.toEntity(email, jobOpeningUrl)
                ).toList();

            // 새로운 Mapping 객체를 한꺼번에 데이터베이스에 저장
            userToJobOpeningMappingRepository.saveAll(userToJobOpeningMappingList);
        });
    }
}