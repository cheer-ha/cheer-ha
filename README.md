...
### 주요 로직
채용공고를 조회할 때 Redis에서 원자적 연산을 통해 조회 수를 집계하고 집계된 조회 수를 채용공고 테이블에 업데이트하는 방식을 사용했습니다.
...
- 낙관적 락을 구현하게 되면 충돌 발생 시 평균적으로 90%의 조회 수가 누락되기 때문에 정합성에 대한 문제를 걱정하지 않을 수 없기에 배제하였고, 비관적 락만 사용하여도 데드락의 가능성이나 응답속도의 불규칙성 등의 이유로 Redis를 집계테이블처럼 사용하고, 분산 락이 적용된 작업 스케줄러를 사용하는 방식을 선택했습니다.

### 회고
Redis를 사용하여 락 없이 조회 수 정합성을 유지하고 분산 락이 적용된 작업 스케줄러를 이용하여 조회 수를 반영하는 로직에서는 조회 수 반영 전에 Redis에 문제가 생긴다면 조회 수 누락이 발생할 가능성이 있습니다. 따라서 이후 추가적인 개발을 한다면 Redis 서버를 늘려서 백업용으로 활용하는 방식을 생각하고 있습니다.
</details>

<details> 
<summary>🔩 JWT만을 사용한 인증, 어떻게 고도화 할 수 있을까요?</summary> 

### 구현한 기능
- 인증-인가에 JWT를 사용하였습니다.
- JWT RefreshToken 구현, JWT Payload 암호화, Redis 블랙리스트 관리

### 주요 로직
- RefreshToken으로 AccessToken을 재발급 받을 수 있습니다.
- 유저가 로그아웃 시 AccessToken이 블랙리스트에 등록되고, RefreshToken은 삭제됩니다.
- JWT Payload는 AES256(CBC) 암호화 됩니다.

### 배경
1. 다중인스턴스 환경에서의 상태 동기화 즉 세션 관리와 세션을 사용하게 되면 Spring security에 대한 학습도 동반되어야 하는 점이 부담되었습니다.
2. 따라서 세션은 베재하고, JWT만을 사용하게 되었습니다.
3. JWT는 세션에 비해 네트워크 비용이 증가할 수 있습니다.
4. JWT는 서버가 상태를 저장하지 않으므로 로그아웃이 어렵습니다.
5. JWT는 토큰 탈취, 토큰 재사용, 토큰 수정 등의 보안적인 위험이 있습니다.

### 요구사항
1. 네트워크 비용을 최소화해야합니다.
2. 다중 인스턴스 환경에서도 모든 사용자의 로그인-로그아웃 상태를 일관되게 유지해야 합니다.
3. 토큰 재사용 공격 등을 최대한 방어해야 합니다.
4. AccessToken의 약점인 짧은 만료시간을 커버해야 합니다.

### 의사결정/사유
1. JWT Payload에 최소한의 정보만 담아 네트워크 비용을 축소시켰습니다.
2. Redis로 RefreshToken과 BlackList를 관리해 다중 인스턴스 환경에서도 일관된 로그인 환경을 유지했습니다.
3. JWT Payload를 암호화하여 토큰 수정을 방지했습니다.
4. RefreshToken으로 AccessToken을 재발급해, AccessToken의 만료 시간 마다 Redis에서 재발급 받을 수 있도록 하여 사용자의 순수 로그인 요청을 줄여 DB부하를 줄였습니다.
</details>
...
### 요약
채용 공고 페이지 조회 시 조회 속도와 응답시간 문제를 개선하고 데드락 문제를 해결했습니다. 

### 문제 정의
채용공고 페이지 조회 시 조회 속도도 느렸습니다.  
동시에 다수가 접근할 시에는 데드락 문제가 발생하여 관리자조차 정보 업데이트가 불가능했습니다.
거기다 응답 속도는 대기에 따라서 변동성이 커서 평균 응답 시간 대비 최악의 응답 시 간이 너무 길다는 문제가 있었습니다.

### 가설
집계를 할 때 비관적 락이 걸리면 조회와 집계가 동시에 이루어질 때는 데드락 문제가 발생하고, 
조회와 집계가 동시에 이루어지지 않는다 해도 비관적 락으로 인해 병목 현상 등이 일어날 수 있었기에 응답속도에 문제가 있는 것이라 추측했습니다.

### 해결 방안
- Redis 자체의 원자적 연산 특성을 이용하여 동시성 문제를 해결했습니다.
- Redis를 집계 테이블처럼 사용하여 채용공고 페이지와의 역할을 분리하여 갱신에 의한 데드락 문제를 해결했습니다.
- 정기적으로 Redis의 조회 수를 채용공고 테이블의 조회 수에 업데이트 해주는 방식으로 해결했습니다.
...
<details> <summary>🧩 다중 인스턴스 환경에서의 스케줄러 동시 실행 문제</summary> 

### 요약
다중 인스턴스 환경에서 스케줄러가 동시에 실행되는 문제를 해결했습니다.

### 문제 정의
![스크린샷 2025-03-04 오후 3 13 01](https://github.com/user-attachments/assets/04ed0d74-5dba-467d-9ed7-a86da292c6d6)
- 다중 인스턴스 환경에서 스케줄러가 인스턴스의 개수만큼 실행되는 문제가 있었습니다.

### 해결 과정
- 일반적인 분산 락으로는 해결 불가능하다고 판단했습니다.
- 분산 락과 DB락은 "동시성 문제" 를 해결하기 위한 플랜이고, 필요한 것은 순차 실행이 아니라 "중복 실행이 방지되는 것" 입니다.
- 따라서 Redis 캐시를 일종의 락처럼 사용하여 해결하였습니다.
![스크린샷 2025-03-04 오후 3 14 45](https://github.com/user-attachments/assets/b9da706b-b304-4c13-8671-666d1b60cb3b)
- 스케줄러의 식별자를 key로, 현재 실행중인 jvm의 고유한 ID를 value로 사용하였습니다.
- 프로젝트에서 실행되는 스케줄러가 많기 때문에 유틸클래스로 분리해서 다른 스케줄러에서도 재사용 가능하게 만들었습니다.

### 추후 계획
이러한 문제를 해소해줄 수 있는 "ShedLock" 라이브러리도 알아봐야 합니다.
