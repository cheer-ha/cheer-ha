...
| **회차** | **채용 공고 조회** | **사용자 조회** | **전체 이메일 전송** | **전체 작업** | **이메일 전송 (1)** | **이메일 전송 (2)** | **이메일 전송 (3)** |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
### 결론
- 채용 공고 조회 속도: 150ms → 100ms로 **33.3% 단축*8
- 전체 이메일 발송 작업: 21s 832ms → 9s 736ms로 **55.4% 향상**
- 전체 작업 완료: 21s 890ms → 9s 777ms로 **55.4% 향상**
...
- **Spring Boot** 기반 애플리케이션으로 **AWS EC2 인스턴스 내의 Docker** 에서 실행
- **ALB 사용**: 사용자 요청을 여러 백엔드 서버로 라우팅하여 부하 분산
- **주기적인 Health Check와 ASG**: 높은 가용성 제공
- **EC2 인스턴스**: Auto Scaling Group에 속하여 트래픽 증가에도 안전하게 대응
</details>

<details>
  <summary> 데이터베이스 및 캐시 </summary>

- **MySQL (RDS)**: 고가용성 및 편리한 파라미터 설정
- **Redis (ElastiCache)**: Redis는 보안이 취약 → **ElastiCache**에서 제공하는 높은 보안 수준으로 관리
- **Elasticsearch**: 신버전을 편리하게 사용할 수 있는 Elastic Cloud 사용 → 편리한 플러그인 추가, 까다로운 Security 설정 필요 ❌
</details>

<details>
  <summary> 네트워크 관리 </summary>

- **Route 53**: 도메인 관리 및 사용자가 접근하는 요청을 **ALB**로 처리하도록 설정
- **ALB에 SSL 인증서 연결**: HTTPS 요청만 처리 → HTTP 요청은 자동으로 HTTPS로 리디렉션됨
- **VPC** 내에서 **메인 애플리케이션의 Auto Scaling Group**, **RDS MySQL**과 **ElastiCache Redis, 모니터링/로깅 서버를** 함께 관리 → 보안 강화, Private IP를 이용하여 통신해 네트워크 연결을 빠르고 효율적으로 처리
- **Web Application Firewall**: 봇, 악성 사용자, SQL injection 공격을 막고 로깅
</details>

<details>
  <summary> 모니터링 및 로깅 </summary>

- Prometheus + Grafana / Fluentd를 VPC 내의 EC2에 각각 도커를 이용해 모니터링-로깅 전용 서버 구축
- 메인 애플리케이션과 Private IP로 통신하므로, 모니터링 정보(exporter)와 로그(logback) 정보 전달 속도가 빠름
</details>
...
- 배포용 브랜치에 **코드가 병합**되면, **GitHub Actions**를 사용하여 **CI 테스트**를 자동으로 실행
- 이 단계에서 **코드의 안정성**을 검증하여, 문제가 있는 코드는 배포 방지 
</details>
...
- 테스트 통과 시 **도커 이미지 빌드**
- 빌드된 도커 이미지는 **Docker Hub**에 **푸시** → 이후 배포 과정에서 사용할 수 있도록 준비
</details>

<details>
  <summary> CD 단계 (배포 자동화) </summary>

- **CD 단계**: **AWS의 Auto Scaling Group**을 활용하여 **롤링 업데이트** 트리거
- 롤링 업데이트: 서비스의 가용성 유지 및 **새로운 버전의 애플리케이션 점진적 배포** → 새로운 인스턴스가 완전히 배포되고 헬스체크 완료 시  구버전 인스턴스 삭제
- 시작 템플릿에서 **Shell Script** 작성 → 완벽한 자동화 완료  
</details>

<details>
  <summary> 무중단 배포 </summary>

- **무중단 배포**(Zero-Downtime Deployment) 방식이 적용되어, 서비스의 **다운타임 없이** 배포 진행
- 롤링 업데이트 방식 선택: 별다른 설정 없이 ASG로 편리하게 사용 가능
- 블루-그린과 비교했을 때 배포 도중 트래픽이 일시적으로 몰리는 현상이 없어 서버 관리 비용 부담이 덜하지만, 배포 중에 신버전과 구버전이 혼재함   
  → 현재 서비스에서는 API가 크게 바뀔 상황이 많지 않고, 모니터링-로깅을 구축해 혹시 모를 상황에 대비하도록 설계  
  → 사용자에게 영향을 주지 않고, 새로운 버전의 애플리케이션을 안전하게 배포 가능  
  → 새로운 버전에 이상이 있을 때에도 버전 롤백을 통해 기존 버전을 그대로 사용 가능  
</details>
</details>
...
<details>
  ...
  ![image.png](https://private-user-images.githubusercontent.com/184512169/423202975-90c807ba-d3e7-421e-b0bd-45ee8c02233a.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTkyODIsIm5iZiI6MTc0MjExODk4MiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTc1LTkwYzgwN2JhLWQzZTctNDIxZS1iMGJkLTQ1ZWU4YzAyMjMzYS5wbmc_.Inhx3fPkXxSEl9EHl_PSF_r_HImqgiiibL55s9rwdYc)
  ...
  ![스크린샷 2025-03-12 오후 4.59.49.png](https://private-user-images.githubusercontent.com/184512169/423202930-13a66037-59b1-4883-9fba-438d46066ef3.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTMwLTEzYTY2MDM3LTU5YjEtNDg4My05ZmJhLTQzOGQ0NjA2NmVmMy5wbmc_.-MhGCXzMTCvBdlvGQSTxptxff9-DD3oSFo3cblipA4c)
  ...
  ![image.png](https://private-user-images.githubusercontent.com/184512169/423202931-89e087d1-ea74-416c-a2d4-c370a3383e73.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTkyODIsIm5iZiI6MTc0MjExODk4MiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTMxLTg5ZTA4N2QxLWVhNzQtNDE2Yy1hMmQ0LWMzNzBhMzM4M2U3My5wbmc_.NKHba01FEmiXpGtpTdneMX1Kmt3xt0Cxfg3Puys3EPc)
  ...
  1. **Grafana + Prometheus / Fluentd를 함께 운용하나, 각각 다른 스택으로 띄워 네트워크 분리**
     ...
     ![image.png](https://private-user-images.githubusercontent.com/184512169/423202926-f89fc425-3e05-414f-8ddb-4273a5fa4f41.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTkyODIsIm5iZiI6MTc0MjExODk4MiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI2LWY4OWZjNDI1LTNlMDUtNDE0Zi04ZGRiLTQyNzNhNWZhNGY0MS5wbmc_.OG0shhEd6oar7kWA9XxxtfWizAOGmnU6ayZBq3u8Nu0)

  2. **모니터링 대상으로 `MySQLld`, `EC2`, `JVM`, `Redis` 선정**

     ![스크린샷 2025-03-12 오후 4.13.23.png](https://private-user-images.githubusercontent.com/184512169/423203086-3bf5be00-fddd-4a74-a5d5-e9ad6cb8a029.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAzMDg2LTNiZjViZTAwLWZkZGQtNGE3NC1hNWQ1LWU5YWQ2Y2I4YTAyOS5wbmc_.O5ZchrsqUbOVPTiKn1gArZiK7-LB_tuJaySVc04lcSU)
  ...
  3. **Logback Appender: `Console`, `Fluentd`만 사용하며, 배포 환경에서 info 이상인 애플리케이션 로그만 표시**
     ...
     ![스크린샷 2025-03-12 오후 4.00.01.png](https://private-user-images.githubusercontent.com/184512169/423202927-9dc66e2a-1a4d-41a5-968f-0bb719b893f0.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI3LTlkYzY2ZTJhLTFhNGQtNDFhNS05NjhmLTBiYjcxOWI4OTNmMC5wbmc_.l-mJFhifq-CK0ZYxptt4CzBMJWGJxi6-va6Kob-hUY8)

  4. **Grafana와 Slack 알림 연동하여 모니터링 효율 강화**
     - 애플리케이션의 CPU, RAM, 500에러 등 체크

      ![스크린샷 2025-03-13 오후 3.27.27.png](https://private-user-images.githubusercontent.com/184512169/423202928-dd60336d-f869-4a5a-a8be-dcc3f4a0d175.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI4LWRkNjAzMzZkLWY4NjktNGE1YS1hOGJlLWRjYzNmNGEwZDE3NS5wbmc_.Z6z9tJ4gGjpa_4zVQlSHgV1R9InTvBQS_bebqOM3lrg)
  ...
  - **[문제 발생]** 네트워크 이슈 발생: 노트북 와이파이 문제로 가용성을 유지하지 못함
      ...
      ![image.png](https://private-user-images.githubusercontent.com/184512169/423202929-00009b43-d3a4-4048-93cf-91f36af474d8.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI5LTAwMDA5YjQzLWQzYTQtNDA0OC05M2NmLTkxZjM2YWY0NzRkOC5wbmc_.ncdF2DrE-hZldHQegp0464jAEnSdnVLmVfv52WLeydg)
        
      ![image.png](https://private-user-images.githubusercontent.com/184512169/423202924-54f1d744-4c1a-4725-a192-78282550c8f4.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI0LTU0ZjFkNzQ0LTRjMWEtNDcyNS1hMTkyLTc4MjgyNTUwYzhmNC5wbmc_.TA6FYQ3V_iX9Eq4Bkep-KzEQoYtVcb4LaHsbYRCiuZI)
        
  - **[해결 방법]** 서버를 EC2로 옮김: Private IP 통신으로 실시간 모니터링, 로깅 가능
        
      ![스크린샷 2025-03-12 오후 4.08.26.png](https://private-user-images.githubusercontent.com/184512169/423202925-c04452cd-2730-4a3f-9cc8-c27b075a8636.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTI1LWMwNDQ1MmNkLTI3MzAtNGEzZi05Y2M4LWMyN2IwNzVhODYzNi5wbmc_.n-cbNQtMM2Br28llv7abg7I6eHRor_3u1DyRAA6nSB8)
        
      ![스크린샷 2025-03-12 오후 4.10.16.png](https://private-user-images.githubusercontent.com/184512169/423202935-ea0b437d-1f78-44f7-9e61-3bba47d8594a.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMTk0MDgsIm5iZiI6MTc0MjExOTEwOCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjAyOTM1LWVhMGI0MzdkLTFmNzgtNDRmNy05ZTYxLTNiYmE0N2Q4NTk0YS5wbmc_.9k20aUyn0ElxP6VpouP5il8tkZnfKIHxHs2GofagSvo)
...
| **TPS** | 249.1건/sec | 35.1건/sec | **7.1배 감소** |
  ...
  ![초당처리량 감소 이미지](https://private-user-images.githubusercontent.com/184512169/423204465-8e05aa97-1887-42d3-ab83-84e8ed2b00ef.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjE3MjcsIm5iZiI6MTc0MjEyMTQyNywicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA0NDY1LThlMDVhYTk3LTE4ODctNDJkMy1hYjgzLTg0ZThlZDJiMDBlZi5wbmc_.ZfWymnagWLNsqj9y3YdZOymRQRVaUvDlIv25RUVwg94)
...
- 스레드 수를 200개로 조정하여 성능이 안정되면서 TPS가 증가함
  ...
  ![초당처리량 증가 이미지](https://private-user-images.githubusercontent.com/184512169/423204330-d4226ade-91e5-46d4-94f9-44c4ec18730b.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjE3MjcsIm5iZiI6MTc0MjEyMTQyNywicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA0MzMwLWQ0MjI2YWRlLTkxZTUtNDZkNC05NGY5LTQ0YzRlYzE4NzMwYi5wbmc_.H6ZQAlu6q-4rqxeNDp5sWwQotUYj2RmD2VPQpbY4gHA)
...
![image.png](https://private-user-images.githubusercontent.com/184512169/423204332-740e8b34-2c5d-4b7a-90cc-3e3b5902f611.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9._.6GooSPaARn-ubjpqAeY_qPU3Jus2KEz7JeYNdqDfLbw)
                
![다운로드 (13) (1).png](https://private-user-images.githubusercontent.com/184512169/423207031-f534246c-aa53-4540-b566-f85268d72aeb.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjM0NTYsIm5iZiI6MTc0MjEyMzE1NiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA3MDMxLWY1MzQyNDZjLWFhNTMtNDU0MC1iNTY2LWY4NTI2OGQ3MmFlYi5wbmc_.kEGyiimWWpJR-QURtnVkzPjJu0Q_urV4kdzrVuUvvYA)
                
![다운로드 (14) (1).png](https://private-user-images.githubusercontent.com/184512169/423207070-fb969df4-673a-4601-af84-6343d67e4c6d.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjM0NTYsIm5iZiI6MTc0MjEyMzE1NiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA3MDcwLWZiOTY5ZGY0LTY3M2EtNDYwMS1hZjg0LTYzNDNkNjdlNGM2ZC5wbmc_.HujeQJeojWf0rjLNMmVKZqboucGgV1QzQNDZ1qESGjI)
...
![다운로드 (15) (1).png](https://private-user-images.githubusercontent.com/184512169/423207075-87f0c948-f0ff-4544-85f8-dcc7519a12d7.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjM0NTYsIm5iZiI6MTc0MjEyMzE1NiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA3MDc1LTg3ZjBjOTQ4LWYwZmYtNDU0NC04NWY4LWRjYzc1MTlhMTJkNy5wbmc_.rO7eWcrsC9L_Pb2n2p2w81LF2EDmYlrGLA3mgEDQ_tw)
...
![image.png](https://private-user-images.githubusercontent.com/184512169/423208303-f8123db3-a186-4585-88bb-5b954e115cec.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjU4MDksIm5iZiI6MTc0MjEyNTUwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4MzAzLWY4MTIzZGIzLWExODYtNDU4NS04OGJiLTViOTU0ZTExNWNlYy5wbmc_.wUlzDj96TErqNEz7wepfmEUtT_UlMBQtW6A7WHm5zAE)

![image.png](https://private-user-images.githubusercontent.com/184512169/423208294-9f3f0a14-3207-4412-88b8-9d3b04cce77a.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjU4MDksIm5iZiI6MTc0MjEyNTUwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4Mjk0LTlmM2YwYTE0LTMyMDctNDQxMi04OGI4LTlkM2IwNGNjZTc3YS5wbmc_.TCuzY-xD0Fx1SIim3POZGT1SfPlcPJiS0988PSrQFHM)
...
![image.png](https://private-user-images.githubusercontent.com/184512169/423208293-30c0c5e1-18ba-4bd2-9989-41bc96773425.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjU4MDksIm5iZiI6MTc0MjEyNTUwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4MjkzLTMwYzBjNWUxLTE4YmEtNGJkMi05OTg5LTQxYmM5Njc3MzQyNS5wbmc_.Oe-h9j6pllqrPDx0M4PZhQr9VKb1BiBN3mFJz1Dpzrk)
...
1. 이메일 인증으로 회원가입, 이메일 알림 구독, 비밀번호 리셋 가능
    ...
    ![스크린샷 2025-03-12 오후 11.16.03.png](https://private-user-images.githubusercontent.com/184512169/423208297-8772548d-1074-4d60-b90d-43054b625f70.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjU4MDksIm5iZiI6MTc0MjEyNTUwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4Mjk3LTg3NzI1NDhkLTEwNzQtNGQ2MC1iOTBkLTQzMDU0YjYyNWY3MC5wbmc_.Hdhf--NaNZ26BKwtvjpIAn9AbPMPtQpFKnwW3fV7DI8)
    
2. Kibana의 로그 대시보드로 이상 사용자의 IP를 확인하고, 지속적으로 올바르지 않은 요청을 보낸 IP는 WAF(Web Application Firewall)에서 차단 가능
    
    ![스크린샷 2025-03-12 오후 10.35.06.png](https://private-user-images.githubusercontent.com/184512169/423208300-a706c8bd-c4fc-48c4-ab89-dac615261b4a.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjU4MDksIm5iZiI6MTc0MjEyNTUwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4MzAwLWE3MDZjOGJkLWM0ZmMtNDhjNC1hYjg5LWRhYzYxNTI2MWI0YS5wbmc_.rp4nsQTYjKRzrErj2xJcAbyVYkBvAd7HB4sI0Fu3Coo)
...
![Spring 으로 검색 시](https://private-user-images.githubusercontent.com/184512169/423208296-bb8d4d69-a7a7-4685-b970-4542f0832c08.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjY0MDksIm5iZiI6MTc0MjEyNjEwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4Mjk2LWJiOGQ0ZDY5LWE3YTctNDY4NS1iOTcwLTQ1NDJmMDgzMmMwOC5wbmc_.wIEYJ-ev4rNqybH_wtjW9hnSevvsCd523hblvdInaKY)

Spring 으로 검색 시

![Java 로 검색 시](https://private-user-images.githubusercontent.com/184512169/423208302-1169dd23-d124-4b38-ae5d-c526475d5fa9.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjY0MDksIm5iZiI6MTc0MjEyNjEwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4MzAyLTExNjlkZDIzLWQxMjQtNGIzOC1hZTVkLWM1MjY0NzVkNWZhOS5wbmc_.0rI2r6YE83JgPFPQvZ2cVK4CvQmbcQk6OJlVsDmDNYU)
...
![수정 전 서치텀에서 제목만 검색이 되는 코드](https://private-user-images.githubusercontent.com/184512169/423208299-366fc53d-8184-422a-bded-cf96239fc32b.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjY0MDksIm5iZiI6MTc0MjEyNjEwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4Mjk5LTM2NmZjNTNkLTgxODQtNDIyYS1iZGVkLWNmOTYyMzlmYzMyYi5wbmc_.MtXgbmls_ktlp7RTTbe0FwTR9KtWt0KYMT0uNQqbuDY)
        
   수정 전 서치텀에서 제목만 검색이 되는 코드
        
![수정 후 우선순위를 매겨 서치텀에서 제목, 회사명, 자격요건이 함께 검색되도록 함](https://private-user-images.githubusercontent.com/184512169/423208301-fca8ddba-23f6-40db-b16b-c519298d3a3c.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjY0MDksIm5iZiI6MTc0MjEyNjEwOSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA4MzAxLWZjYThkZGJhLTIzZjYtNDBkYi1iMTZiLWM1MTkyOThkM2EzYy5wbmc_.5tSacua-k_TES6wa4ywP4tdvo_J1y4wXl44ZO93uqDI)
...
<summary> 예시 사진 모음 </summary>
   ...
   ![](https://private-user-images.githubusercontent.com/184512169/423210227-f33393cd-841e-4d1a-91c7-aae80b333f09.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYwODQsIm5iZiI6MTc0MjEyNTc4NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwMjI3LWYzMzM5M2NkLTg0MWUtNGQxYS05MWM3LWFhZTgwYjMzM2YwOS5wbmc_.VsQO3cFssxTFN_ZF6ziaMkP9QyTkylO_LFMI2bdzzV0)

   (2) 모부스로 검색 시 모비스를 찾아옴

   ![](https://private-user-images.githubusercontent.com/184512169/423210226-e48a770f-609e-429e-ac31-962457232eb6.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYwODQsIm5iZiI6MTc0MjEyNTc4NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwMjI2LWU0OGE3NzBmLTYwOWUtNDI5ZS1hYzMxLTk2MjQ1NzIzMmViNi5wbmc_.zj74NXfOvVW4_bbXzZZj2RP8lx_awi3kxIhJfDebNRM)

   (3) 쿠카오로 검색 시 카카오를 찾아옴

   ![](https://private-user-images.githubusercontent.com/184512169/423210235-1254ba70-49fa-4614-b4ee-edf43b3dac18.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYwODQsIm5iZiI6MTc0MjEyNTc4NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwMjM1LTEyNTRiYTcwLTQ5ZmEtNDYxNC1iNGVlLWVkZjQzYjNkYWMxOC5wbmc_.dY_RjuskLSje73zeW7pZQoYjvdBtfEUl0BkeTZ-P2Rs)

   (4) 나이버로 검색 시 네이버를 찾아옴

   ![](https://private-user-images.githubusercontent.com/184512169/423210236-b21c31b7-7d98-40c4-b9b7-d790fd87e137.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYwODQsIm5iZiI6MTc0MjEyNTc4NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwMjM2LWIyMWMzMWI3LTdkOTgtNDBjNC1iOWI3LWQ3OTBmZDg3ZTEzNy5wbmc_.V93V4v3wMwNWJ4pf-Mm9-R-hs2fPnQbabAQiE_D3vSs)

   (5) 네이바로 검색 시 네이버를 찾아옴

   ![](https://private-user-images.githubusercontent.com/184512169/423210237-f1386b6a-1ef2-45b4-b19a-dde2f3abd3de.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYwODQsIm5iZiI6MTc0MjEyNTc4NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwMjM3LWYxMzg2YjZhLTFlZjItNDViNC1iMTlhLWRkZTJmM2FiZDNkZS5wbmc_.-QG8wSx75cQbGQiQjPcR5BQdbx_x-w7OFzoRskKRKZ4)
  ...
  - 기본 검색 (must 사용)
      ...
      ![](https://private-user-images.githubusercontent.com/184512169/423210906-0a0fe956-72a8-462e-9016-eb38378aba18.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYzNTUsIm5iZiI6MTc0MjEyNjA1NSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwOTA2LTBhMGZlOTU2LTcyYTgtNDYyZS05MDE2LWViMzgzNzhhYmExOC5wbmc_.BQ6c8v4-v4hJy1SRLFZnoW8LUoZsYOyPlCcHMk7mFVI)
        
  - **`Fuzzy`** 기능 사용
        - **`fuzziness`**: 자동으로 오타 및 변형된 단어를 감지하고 유사한 단어를 검색 결과에 포함하도록 설정
        - **`should`** 조건: 하나 이상의 조건이 만족되면 결과가 반환되도록 조건 추가
        
      ![](https://private-user-images.githubusercontent.com/184512169/423210905-1bb8e845-9b77-4780-9cbb-1ed602b079f5.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjYzNTUsIm5iZiI6MTc0MjEyNjA1NSwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEwOTA1LTFiYjhlODQ1LTliNzctNDc4MC05Y2JiLTFlZDYwMmIwNzlmNS5wbmc_.dhx5QxIiYXMjE3aKXskZCHZaln-ffQTaW1757T2i0rc)
...
<summary> 예시 사진 모음 </summary>
   ...
   ![image.png](https://private-user-images.githubusercontent.com/184512169/423209123-00b93908-761e-4ec0-8f36-53fb4fafa5f7.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjUyOTIsIm5iZiI6MTc0MjEyNDk5MiwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjA5MTIzLTAwYjkzOTA4LTc2MWUtNGVjMC04ZjM2LTUzZmI0ZmFmYTVmNy5wbmc_.l5iIbmf9-KXYUrfZa0_7JtnaNyqe2QFwqHuh2VyJ0n4)
...
![스크린샷 2025-03-13 오후 11.54.52.png](https://private-user-images.githubusercontent.com/184512169/423212755-4445c13e-d4a9-4593-a336-ff922071d6d4.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3NDIxMjc0NTQsIm5iZiI6MTc0MjEyNzE1NCwicGF0aCI6Ii8xODQ1MTIxNjkvNDIzMjEyNzU1LTQ0NDVjMTNlLWQ0YTktNDU5My1hMzM2LWZmOTIyMDcxZDZkNC5wbmc_.3FwEPIDMXV4tVqxGv5i4474Vjj4lqTG68S1DcE6p0CQ)
